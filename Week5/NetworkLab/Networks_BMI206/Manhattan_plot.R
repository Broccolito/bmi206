...
pdf("MS Manhattan_plot.pdf",width=12,height=6)
...
### Make mplot for the other GWAS
rm(list=ls())

library("ggplot2")
library("RColorBrewer")

wtcc <- read.table("HT.pvals.out", header=T, as.is=T)
...
pdf("HT Manhattan_plot.pdf",width=12,height=6)
p <- ggplot(wtcc, aes(Start, Discovery_log))+geom_point(size=1.5,alpha=0.6,aes(colour=as.factor(Color_Dis)))+
  scale_colour_manual(values = colours) +
  #scale_color_brewer(palette="Set1")+
  geom_hline(yintercept=-log10(0.05),size=0.5, colour="gray")+
